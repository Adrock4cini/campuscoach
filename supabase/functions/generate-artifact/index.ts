...
{ "items": [ {
  "target": string,
  "mnemonic": string,
  "technique": "acronym"|"association"|"rhyme"|"story"|"chunking"|"visual"|"other",
  "explanation": string,
  "conceptId": string,
  "conceptName": string
} ] }
...
async function callGateway(
  ...
  const feedbackInstruction = body.kind === "mnemonic" && mnemonicPreferences.hasFeedback
    ? `Student-confirmed technique feedback (soft preference only): ${JSON.stringify({
        preferred: mnemonicPreferences.preferred,
        avoid: mnemonicPreferences.avoid,
      })}`
    : null;
