...
describe("incremental study evidence", () => {
  it("only sends what has not been saved yet", () => {
    ...
    expect(pendingEvidenceSegment(results, 0)).toMatchObject({ total: 3, correct: 2 });
    expect(pendingEvidenceSegment(results, 2)).toMatchObject({ total: 1, correct: 1 });
    expect(pendingEvidenceSegment(results, 3)).toBeNull();
  });
