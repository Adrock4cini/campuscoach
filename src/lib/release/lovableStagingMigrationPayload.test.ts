...
describe("Lovable Cloud staging migration payloads", () => {
  ...
  it("builds one deterministic, exact 65-file manifest", () => {
    ...
    expect(new Set(manifest.entries.map(({ version }) => version)).size).toBe(65);
    expect(new Set(manifest.entries.map(({ filename }) => filename)).size).toBe(65);
  ...
  it("creates a private control schema and never impersonates Supabase's ledger", () => {
    ...
    expect(payload.match(/^ {2}\(\d+, '\d{14}',/gmu)).toHaveLength(65);
    for (const entry of manifest.entries) {
  ...
  it("encodes the exact 14 documented post-phase gates", () => {
