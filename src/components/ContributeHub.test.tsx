...
describe("ContributeHub demo submissions", () => {
  ...
  it("labels the flow as local and reports that a sample was not shared", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /add professor hint/i }));
    ...
    expect(await screen.findByText(/Demo only · Professor hint: Memory models\. Not saved or shared\./i)).toBeInTheDocument();
  });
});
