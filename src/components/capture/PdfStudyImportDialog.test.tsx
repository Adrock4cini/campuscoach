...
const mocks = vi.hoisted(() => ({ owner: "owner", hash: "a".repeat(64), commit: vi.fn(), retry: vi.fn(), render: vi.fn(), destroy: vi.fn(), close: vi.fn() }));
...
vi.mock("@/lib/capture/pdfRenderer", () => ({ openPdf: async () => ({ fileName: "Handbook.pdf", fileHash: mocks.hash, pages: 8, render: mocks.render, destroy: mocks.destroy }) }));
...
async function choosePdf() {
  fireEvent.change(screen.getByLabelText(/PDF \(up to|Reselect PDF to resume/), { target: { files: [new File(["%PDF-test"], "Handbook.pdf", { type: "application/pdf" })] } });
  await screen.findByText("Handbook.pdf · 8 PDF pages");
}
beforeEach(() => {
  vi.clearAllMocks(); localStorage.clear(); mocks.owner = "owner"; mocks.hash = "a".repeat(64);
...
describe("PDF study import UI", () => {
  ...
  it("refuses a different reselected PDF without changing the saved attempt IDs", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Reselect PDF to resume"), { target: { files: [new File(["different"], "Other.pdf")] } });
    await screen.findByText("To resume, select the same PDF: Handbook.pdf.");
