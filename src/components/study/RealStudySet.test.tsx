...
describe("real study set freshness", () => {
  ...
  it("explains the strongest saved reasons for a smart selection", () => {
    mocks.artifact = {
      ...artifact(CURRENT_ARTIFACT_PROMPT_VERSION),
      study_scope_snapshot: {
        selectionEvidence: [
    ...
    expect(screen.getByText(/why these concepts:/i)).toBeInTheDocument();
    expect(screen.getByText(/cell division:/i).closest("li")).toHaveTextContent(
      "Cell division: Named in the test topics · Review is due",
    );
    expect(screen.getByText(/DNA:/i).closest("li")).toHaveTextContent(
      "DNA: Low mastery · Teacher emphasized it",
    );
    expect(screen.queryByText(/extra hidden reason/i)).not.toBeInTheDocument();
    expect(screen.getByText("RNA:").parentElement).toHaveTextContent("RNA: Not studied yet");
    expect(screen.getByText("Protein:").parentElement).toHaveTextContent("Protein: Review is due");
  });
