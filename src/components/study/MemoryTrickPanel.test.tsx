...
describe("MemoryTrickPanel", () => {
  ...
  it("loads an existing one-concept artifact and separates truth from the trick", async () => {
    ...
    expect(screen.getByText("AI-created memory trick")).toBeInTheDocument();
    expect(screen.getByText("Technique: Association")).toBeInTheDocument();
  ...
  it("exposes feedback callbacks without treating a view as mastery", async () => {
    ...
    expect(screen.getByText(/does not change mastery/i)).toBeInTheDocument();
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "Try another" })).toBeEnabled());
  });
